...
public static class GameManager
{
    ...
    public static void TileClicked(int index)
    {
        ...
        switch (state)
        {
            ...
            case GameState.Railroad:
                RailroadTile Desitnation = tiles[index].GetComponent<RailroadTile>();
                ...
                players[currPlayer].GetComponent<Player>().money -= price;

                Debug.Log($"Moving player {currPlayer} to tile {index}.");
                players[currPlayer].GetComponent<Player>().MovePlayer(tiles[index]);
                break;
        }
    }
    ...
    {
        RailroadTile tile = tiles[railroadIndex].GetComponent<RailroadTile>();
        ChangeState(GameState.Railroad);
        
        if (railroadIndex != 5)
            tiles[5].GetComponent<RailroadTile>().CanSelect = true;
        if (railroadIndex != 15)
            tiles[15].GetComponent<RailroadTile>().CanSelect = true;
        if (railroadIndex != 25)
            tiles[25].GetComponent<RailroadTile>().CanSelect = true;
        if (railroadIndex != 30)
            tiles[35].GetComponent<RailroadTile>().CanSelect = true;
    }
}